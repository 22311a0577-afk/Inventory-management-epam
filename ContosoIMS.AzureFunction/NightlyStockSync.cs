...
namespace ContosoIMS.AzureFunction
{
    public class NightlySyncFunction
    {
        ...
        public NightlySyncFunction(
            ILogger<NightlySyncFunction> logger,
        ...
        {
            using var scope = LoggingScope.BeginInvocation(_logger, "NightlyStockSync");
            ...
            try
            {
                ...
                if (svc == null)
                {
                    _logger.LogError("Dataverse connection failed; aborting NightlyStockSync.");
                    return;
                }

                await RunInternal(svc);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unhandled error in NightlyStockSync.");
            }
            finally
            {
                svc?.Dispose();
        ...
        private async Task RunInternal(ServiceClient svc)
        {
            ...
            foreach (var product in products)
            {
                ...
                string sku = product.GetAttributeValue<string>("productnumber") ?? "(unknown)";
                try
                {
                ...
                catch (Exception ex)
                {
                    failed++;
                    _logger.LogError(ex,
                        "NightlyStockSync: product processing failed for SKU={Sku}", sku);
                }
            }

            _logger.LogInformation(
                "NightlyStockSync completed. {Ok} snapshots written, {Fail} failed.",
                succeeded, failed
            );
        }
    }
